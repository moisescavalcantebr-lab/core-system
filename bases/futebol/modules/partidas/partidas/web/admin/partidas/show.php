...
try {
    ...
    $stmt->execute([$id]);
    $hasOtherLiveMatch = (int)$stmt->fetchColumn() > 0;
} catch (Throwable $e) {
    $hasOtherLiveMatch = false;
}
    ...
    <div class="c-page-header">
        ...
        <div>
            <?php if (($match['status'] ?? '') === 'scheduled'): ?>
                ...
                <?php else: ?>
                    <button class="c-btn-secondary" type="button" disabled title="Finalize a partida em andamento antes de iniciar outra.">Iniciar</button>
                <?php endif; ?>
            <?php endif; ?>
