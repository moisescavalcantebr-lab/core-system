                    ...
                    <?php else: ?>
                        <div class="c-lineup-avatar-grid">
                            <?php foreach ($cardPlayers as $player): ?>
                                <form action="<?= PROJECT_URL ?>/admin/partidas/lineup_store.php?id=<?= (int)$match['id'] ?>" method="POST">
                                    <?= csrf_field(); ?>
                                    <input type="hidden" name="player_id" value="<?= (int)$player['id'] ?>">
                                    <button class="c-lineup-avatar-card">
                                    ...
                                    <select name="override_position_id" class="c-lineup-position-select" aria-label="Posição nesta partida">
                                        <option value="">Posição automática</option>
...
.c-lineup-position-select { width: 100%; margin-top: 6px; border: 1px solid rgba(148,163,184,.28); background: rgba(15,23,42,.72); color: #e5e7eb; padding: 6px; font-size: 10px; }
...
@media (max-width: 700px) { .c-lineup-avatar-grid, .c-lineup-reserve-grid, .c-lineup-inline-info .c-lineup-info-list { grid-template-columns: repeat(2, minmax(0, 1fr)); } .c-lineup-token form { opacity:1; } }
</style>
