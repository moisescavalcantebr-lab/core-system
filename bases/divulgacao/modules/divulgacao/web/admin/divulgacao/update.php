...
$currentStmt = $pdo->prepare('SELECT offer_image, offer_image_2 FROM divulgacao_pages WHERE id = ? LIMIT 1');
$currentStmt->execute([$id]);
$currentPage = $currentStmt->fetch(PDO::FETCH_ASSOC) ?: [];
$offerImage = (string)($currentPage['offer_image'] ?? '');
$offerImage2 = (string)($currentPage['offer_image_2'] ?? '');
if (!empty($_POST['remove_offer_image'])) {
    $offerImage = '';
}
if (!empty($_POST['remove_offer_image_2'])) {
    $offerImage2 = '';
}
$uploadedImage = isset($_FILES['offer_image']) && is_array($_FILES['offer_image']) ? divulgacaoUploadOfferImage($_FILES['offer_image']) : '';
if ($uploadedImage !== '') {
    $offerImage = $uploadedImage;
}
$uploadedImage2 = isset($_FILES['offer_image_2']) && is_array($_FILES['offer_image_2']) ? divulgacaoUploadOfferImage($_FILES['offer_image_2']) : '';
if ($uploadedImage2 !== '') {
    $offerImage2 = $uploadedImage2;
}

if ($id <= 0 || $title === '' || $headline === '') {
...
try {
    ...
    redirect(PROJECT_URL . '/admin/divulgacao/edit.php?id=' . $id);
} catch (Throwable $e) {
    flash('error', 'Nao foi possivel salvar. Verifique se o slug ja existe.');
    redirect(PROJECT_URL . '/admin/divulgacao/edit.php?id=' . $id);
}
