...
$stmt = $pdo->prepare("
    SELECT
        ...
        c.context AS competition_context,
...
function playerResultLabel(array $result): string
{
...
function playerResultClass(array $result): string
{
    return match (playerResultLabel($result)) {
                        ...
                        <tbody>
                            <?php foreach ($results as $result): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="c-result-score <?= playerResultClass($result) ?>">
                                            <?= (int)$result['score_a'] ?> x <?= (int)$result['score_b'] ?>
                                        </span>
                                        <span class="c-result-label"><?= playerResultLabel($result) ?></span>
                                    </td>
...
.c-result-card-pill--yellow {
    ...
    color: color-mix(in srgb, var(--warning-color, #f59e0b) 82%, var(--text-primary));
}

.c-result-card-pill--red {
    background: color-mix(in srgb, var(--danger-color, #ef4444) 18%, transparent);
    color: color-mix(in srgb, var(--danger-color, #ef4444) 82%, var(--text-primary));
}
...
.c-result-score--win {
    ...
    color: color-mix(in srgb, var(--success-color, #22c55e) 78%, var(--text-primary));
}
...
.c-result-score--loss {
    ...
    color: color-mix(in srgb, var(--danger-color, #ef4444) 78%, var(--text-primary));
}
